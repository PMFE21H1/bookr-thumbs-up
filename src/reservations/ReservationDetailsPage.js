...
export default class ReservationDetailsPage extends React.Component {
  ...
  componentDidMount() {
    fetch(
      ...
      .then(() =>
        fetch(
          `https://bookr-thumbs-up-default-rtdb.europe-west1.firebasedatabase.app/resources/${this.state.reservation.resource}.json`
        )
      )
      .then((resp) => resp.json())
      .then((resource) => this.setState({ resourcename: resource.name }));
  }
